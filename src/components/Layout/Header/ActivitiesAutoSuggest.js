...
const ActivitiesAutoSuggest = ({
  ...
  const getSuggestionValue = (suggestion) => {
    return suggestion.name[lang];
  };

  const renderSuggestion = (suggestion) => {
    return <div className="suggest_value">{suggestion.name[lang]}</div>;
  };
  ...
  const getSuggestions = (value) => {
    ...
    return inputLength === 0
      ...
      : suggestData.filter(
          (item) =>
            item.name[lang].toLowerCase().slice(0, inputLength) === inputValue
        );
  };
...
export default ActivitiesAutoSuggest;
